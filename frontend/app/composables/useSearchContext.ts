...
interface SearchContext {
  ...
  updateTagFilters: (
    newTagFilters: Array<{ label: string; value: string }>
  ) => void;
...
export function provideSearchContext() {
  const context: SearchContext = {
    ...
    updateTagFilters: (
      ...
      const validTagFilters = newTagFilters.filter(
        (tag) =>
          typeof tag === "object" &&
    ...
    removeTagFilters: (tagToRemove: { value: string }) => {
      if (
        typeof tagToRemove !== "object" ||
        typeof tagToRemove.value !== "string"
      ) {
        console.error(
          "Invalid input: tagToRemove must be an object with a string value"
        );
        return;
      }
      context.tagFilters.value = context.tagFilters.value.filter(
  ...
  const performScrollPull = async () => {
    console.log("Performing scroll pull");
    context.isLoading.value = true;
    context.error.value = null;
    try {
      const filters = context.tagFilters.value.map((tag) => tag.label);
      const data = await scrollChunks(filters);
      context.results.value = data;
    } catch (err) {
